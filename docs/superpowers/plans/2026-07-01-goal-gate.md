...
### Task 7: e2e — goal select, partial tag, persistence

**Files:**
- Create: `e2e/goal-gate.spec.ts`

- [ ] **Step 1: Write the spec**

Create `e2e/goal-gate.spec.ts`, mirroring the serial-suite pattern of `e2e/analysis-card.spec.ts` (same helpers):
...
Run: `bunx playwright test e2e/goal-gate.spec.ts`
Expected: 3/3 pass. If the partial-tag locator is ambiguous (strict-mode violation), scope it: `page.locator("button", { hasText: /analysis/i }).getByText(/·\s*partial/i)` — but prefer the simple form if it passes.

- [ ] **Step 3: Full e2e suite**

Run: `bun run test:e2e`
Expected: all pass (89 pre-existing + 3 new). The pre-existing `analysis-card.spec.ts` must still pass — the card header/footnote changes from Task 4 must not have broken its locators (it matches "Analysis" header text and MEV labels, which Task 4 does not touch).

- [ ] **Step 4: Commit**

```bash
git add e2e/goal-gate.spec.ts
git commit -m "test(e2e): goal select, partial grading tag, goal persistence"
```
